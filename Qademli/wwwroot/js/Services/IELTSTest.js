...
var deleteUser = (id) => {
    ...
    if (xhr.status === 200) {
        LoadData();
        $('#myModal3').modal('hide');
        $.notify("IELTS Test Deleted Successfully", "success")
    }
    else if (xhr.status === 404) {
        LoadData();
        $('#myModal3').modal('hide');
        $.notify("IELTS Test Id not found. Unable to delete " + xhr.status, "error");
...
$('#myModal form').validate({
    ...
    submitHandler: function (form) {
        ...
        if (xhr.status === 200) {
            LoadData();
            $('#myModal').modal('hide');
            $.notify("IELTS Test Added Successfully", "success")
...
$('#myModal2 form').validate({
    ...
    submitHandler: function (form) {
        ...
        if (xhr.status === 200) {
            LoadData();
            $('#myModal2').modal('hide');
            $.notify("IELTS Test Updated Successfully", "success")

        } else if (xhr.status === 404) {
            LoadData();
            $('#myModal2').modal('hide');
            $.notify("IELTS Test Id not found. Unable to update", "error")
        } else {
            $.notify("Your Request Return " + xhr, "error");
        }
