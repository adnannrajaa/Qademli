...
namespace Qademli.AreasAPI.UserApi.Controllers
{
    ...
    public class UserPersonalDetailController : ControllerBase
    {
        ...
        {
            var userId = await _context.UserPersonalDetail.FirstOrDefaultAsync(s => s.UserID == id);
            if (userId == null)
            {
                return NotFound();
            }

            var userPersonalDetail = await _context.UserPersonalDetail.FindAsync(userId.ID);
